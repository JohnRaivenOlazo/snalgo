...
import { knapsackAlgorithm, tspNearestNeighbor } from "@/components/game/utils/algorithms";
import { Play, Pause, RotateCcw, ChevronUp, ChevronDown, ChevronLeft, ChevronRight, Star, HeartPulse, Trophy, Lightbulb, Coins } from "lucide-react";
...
const GameBoard: React.FC = () => {
  ...
  const [foodEatenSinceLastCollectible, setFoodEatenSinceLastCollectible] = useState<number>(0);
  ...
  const startGame = useCallback(() => {
    if (gameState === GameState.READY || gameState === GameState.GAME_OVER || gameState === GameState.WIN) {
      initGame();
    }
  ...
  const handleCapacityChange = useCallback((newCapacity: number) => {
    ...
    if (stats.inventoryCurrentWeight > newCapacity && inventory.length > 0) {
      ...
      const collectibleItems: FoodItem[] = inventoryCollectibles.map(item => ({
        id: item.id,
        position: { x: 0, y: 0 },
      ...
      const totalWeight = updatedInventory.reduce((sum, item) => sum + item.weight, 0);
  ...
  }, [inventory, stats.inventoryCurrentWeight, stats.inventoryCapacity, stats.coinsCollected]);
  
  const updateHintPath = useCallback((head: Position) => {
    if ((food.length === 0 && collectibles.length === 0) || gameState !== GameState.PLAYING) return;
  ...
  const updateGameState = useCallback(() => {
    ...
    if (eatenFood) {
      ...
      if (newFoodEatenCount >= COLLECTIBLE_THRESHOLD && collectibles.length < MAX_COLLECTIBLES) {
        const newCollectible = generateCollectible(newSnake, newFood, collectibles);
    ...
    if (eatenCollectible) {
      ...
      const newInventoryWeight = stats.inventoryCurrentWeight + eatenCollectible.weight;
  ...
  }, [gameState, nextDirection, snake, food, collectibles, stats.inventoryCapacity, stats.inventoryCurrentWeight, foodEatenSinceLastCollectible, gameOver, checkWinCondition, updateHintPath]);
          ...
          <div className="flex justify-between mt-4">
            {gameState === GameState.PLAYING ? (
              ...
              <PixelButton onClick={startGame}>
                ...
                {gameState === GameState.READY || gameState === GameState.GAME_OVER || gameState === GameState.WIN
                  ? "Start"
                  : "Resume"}
              </PixelButton>
            )}
        ...
        <div className="relative">
          ...
          >
            ...
            >
                  ...
                  >
                    ...
                    {hint.map((pos, i) => (
                      <circle
                        ...
                        r={i === 0 ? 4 : 3}
                        fill={i === 0 ? "rgba(255, 255, 255, 0.5)" : "rgba(255, 255, 255, 0.3)"}
                      />
                    ))}
              ...
              {gameState !== GameState.PLAYING && (
                <div className="absolute top-0 left-0 w-full h-full bg-black bg-opacity-60 z-50 flex flex-col items-center justify-center transition-all duration-300 animate-fade-in">
